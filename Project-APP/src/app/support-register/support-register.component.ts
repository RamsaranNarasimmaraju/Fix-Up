...
@Component({
  selector: 'app-support-register',
  templateUrl: './support-register.component.html',
  styleUrls: ['./support-register.component.css'],
})
export class SupportRegisterComponent {
  ...
  user: UserCreateDto = {
    ...
    email: '',
    userPassword: '',
    roleId: 3, // RoleId 3 signifies support
  };
  ...
  register(form: NgForm): void {
    if (form.valid && this.passwordsMatch() && this.usernameValid() && this.passwordValid() && !this.usernameTaken) {
      console.log('Support User Payload:', this.user);

      this.userService.createUser(this.user).subscribe({
        next: (response) => {
          this.successMessage = 'Registration successful! Redirecting to Support Dashboard...';
          this.errorMessage = null;

          form.reset();
          this.user = { userName: '', email: '', userPassword: '', roleId: 3 };
